...
render("Coryne_BGC_Alignments_NRPS.rmd", c("html_document"))
...
fastafh = fastafh[grepl("NRPS", fastafh)]
...
# NRPS BGC proto_gene sequence alignments

## NRPS - All

Unique sequences from all NRPS clusters 
...
msaplot(p=ggtree(tr), fasta = "aln.fasta", offset =1.1, width = 4) + geom_tiplab( hjust = -0.1)
```


## NRPS - non Contig Edges

Unique sequences from NRPS clusters that do not overlap contig edges

```{r nrps_nce_1a, echo = FALSE, results = "asis",fig.width = 20, fig.height =6,warning = FALSE, message = FALSE }

aln <- alnmts[["NRPSno_CEs"]]
aln1 <- msaConvert(aln, "seqinr::alignment")
writeXStringSet(as(aln, "AAStringSet"), file="aln.fasta")
```
```{r nrps_nce_2, echo = FALSE, results = "asis",fig.width = 10, fig.height =8 }
...
```{r nrps_nce_6, echo = FALSE, results = "asis", echo = FALSE, fig.width = 10, fig.height =10,warning = FALSE, message = FALSE}
...
fastatemp <-fastadata[["NRPSno_CEs"]]
...
NRPS_downselection <- data.frame(ct)
NRPS_downselection$Selected<- ifelse(row.names(NRPS_downselection) %in% subsample$SelectedID, "Yes", "No")

NRPS_downselection <- merge(NRPS_downselection, pcinfo, by.x = "row.names", by.y = "SeqID", all.x = TRUE)

outfh = "../../Data/BGC/MultipleAlignments/NRPS_list.csv"
write.csv(NRPS_downselection, outfh, row.names = FALSE)

cat("<br>NRPS down selection results saved to:", outfh)
```
...
```{r NRPS_like_1a, echo = FALSE, results = "asis",fig.width = 4, fig.height =6,warning = FALSE, message = FALSE }

aln <- alnmts[["NRPS_like"]]
aln1 <- msaConvert(aln, "seqinr::alignment")
writeXStringSet(as(aln, "AAStringSet"), file="aln.fasta")
```


```{r NRPS_like_6, echo = FALSE, results = "asis", echo = FALSE, fig.width = 10, fig.height =4,warning = FALSE, message = FALSE}

cat("<br>Tree using Fitch similarity for amino acid sequences")
d <- dist.alignment(aln1, "similarity")
tr <- hclust(d)
#https://4va.github.io/biodatasci/r-ggtree.html
msaplot(p=ggtree(tr), fasta = "aln.fasta", offset =.9, width = 4) + geom_tiplab( hjust = -0.1)
```




## NRPS like - non Contig Edges
Unique sequences from NRPS clusters that do not overlap contig edges

```{r NRPS_nce_like_1a, echo = FALSE, results = "asis", warning = FALSE, message = FALSE }

aln <- alnmts[["NRPS_likeno_CEs"]]
aln1 <- msaConvert(aln, "seqinr::alignment")
writeXStringSet(as(aln, "AAStringSet"), file="aln.fasta")
```

```{r NRPS_nce_like_6, echo = FALSE, results = "asis", echo = FALSE, fig.width = 10, fig.height =4,warning = FALSE, message = FALSE}
