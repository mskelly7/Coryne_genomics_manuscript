...
packages <- c("reshape", "ggplot2", "cowplot", "dplyr", "gggenes", "groupdata2", "svglite", "RColorBrewer", "egg")
...
map <- read.csv('gene_group_color.csv')
...
df_regions1 <- merge(df_regions1, map, by.x="refseq_gene", by.y = "gene", all.x=T)
df_regions1$color[which(is.na(df_regions1$color))] <- "#e4eaf0"
df_regions1$gene_group[which(is.na(df_regions1$gene_group))] <- "other genes"
...
if (region_chunk_size <= 10){
  df_scaling <- df_scaling%>%filter(unique_region_count == 10) # 10
} else if (region_chunk_size <= 20) {
  df_scaling <- df_scaling%>%filter(unique_region_count == 20) # 20
} else if (region_chunk_size <= 30) {
  df_scaling <- df_scaling%>%filter(unique_region_count == 30) # 30
} else if (region_chunk_size >= 31) {
  df_scaling <- df_scaling%>%filter(unique_region_count == 40) # 40
} 
...
for (ch in unique(df_chunk_order$chunk_group)){
  ...
  p1_ggregions <- ggplot(df_regions1 %>% filter(dbscan_label%in%label_keep),
                         aes( xmin=norm_start, xmax=norm_end, y = dbscan_label_dummy, fill=as.factor(gene_group), label = as.character(refseq_gene), forward=strand))+
    ...
    scale_fill_manual(values = c("Other genes"="#c7c1c1", 
                                 "Additional biosynthetic genes"="#FB9A99", 
                                 "Core biosynthetic genes"="#D53E4F", # #9E0142" #D53E4F
                                 "Transport-related genes"="#A6CEE3", 
                                 "Regulatory genes"="#33A02C") ) +
    ...
    scale_y_discrete(#labels = df_chunk_order%>%filter(chunk_group==ch)%>%mutate(region_order=paste(region_order,'. c',dbscan_label,sep=''))%>%select(region_order)%>%pull(),
      labels = df_chunk_order%>%filter(chunk_group==ch)%>%mutate(region_order=paste(ifelse(dbscan_label != -1, 'Group', "Unbinned"), ifelse(as.numeric(dbscan_label) != -1, as.numeric(dbscan_label)+1, "")))%>%select(region_order)%>%pull(),
  ...
  p2_ggdis <- ggplot(df_crm%>%filter(dbscan_label%in%label_keep),
    ...
    geom_boxplot(outlier.shape=NA,alpha=0.5,position='dodge',aes(fill=measurement), show.legend = FALSE)+
    geom_point(position=position_jitterdodge(),aes(color=measurement))+scale_color_brewer(palette="Dark2", labels = c("Pair-wise average", "To representative member")) +
    ...
    ylab(label='Group member counts')+
    xlab(label='Dissimilarity') +
    scale_fill_brewer(palette="Dark2") + 
    labs(color=NULL) +
    guides(color=guide_legend(title=NULL, nrow=2, ncol=1, byrow=TRUE))
  ...
  p3_ggid <- ggplot(df_id%>%filter(dbscan_label%in%label_keep),
    ...
    geom_boxplot(outlier.shape=NA,alpha=0.5,position='dodge',aes(fill=variable), show.legend = FALSE) + #scale_color_manual(values = c("#1B9E77", "#D95F02")) +
    geom_point(position=position_jitterdodge(),aes(color=variable)) + scale_color_brewer(palette="Dark2", labels = c("Identity", "Coverage")) + 
    #geom_point(position=position_jitterdodge(),aes(color=variable)) + scale_color_manual(values = c("#D95F02", "#1B9E77"), labels = c("Identity", "Coverage")) + 
    facet_wrap(~dbscan_label_dummy, ncol=1, scales='free_y')+
    theme_classic()+
    theme(legend.text=element_text(size=12),
  ...
  #p7 <- plot_grid(p1_ggregions, p3_ggid, p2_ggdis,align='hv',nrow=1,rel_widths = c(0.5, 0.25, 0.25), axis='bt', labels = c("A", "B", "C"))
  
  p7 <- egg::ggarrange(p1_ggregions, p3_ggid, p2_ggdis, ncol=3, widths = c(0.5, 0.25, 0.25), 
                       labels = c("A", "B", "C"), 
                       label.args = list(vjust=1.1, hjust=-0.5, 
                                         gp = grid::gpar(font = 1, cex =2))) 
  
  save_plot(paste(visualizations_dir,'/group_summary_', ch,'.',image_format,sep='') ,p7,base_height=6,base_aspect_ratio = 2)
}
